...
try {
  ...
  console.log("Unlock sent via orderbook.", messageId);
  console.log("Orderbook Result:", JSON.stringify(result, null, 2));

  const unlockOk = await findActionInRecentResults(VAULT_PROCESS, "Unlock-OK");
  if (unlockOk) {
    console.log("Vault Unlock-OK:", JSON.stringify(unlockOk, null, 2));
  }
} catch (err) {
