...
const USER = "USER_WALLET_ADDRESS_HERE";
...
try {
  ...
  console.log("Lock sent via orderbook.", messageId);
  console.log("Orderbook Result:", JSON.stringify(result, null, 2));

  const lockOk = await findActionInRecentResults(VAULT_PROCESS, "Lock-OK");
  if (lockOk) {
    console.log("Vault Lock-OK:", JSON.stringify(lockOk, null, 2));
  }
} catch (err) {
